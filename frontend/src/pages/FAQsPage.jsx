    ...
    <div>
      ...
      <div style={{ background: 'var(--bg-secondary)', borderBottom: '1px solid var(--border-subtle)', padding: '1.5rem 1rem' }}>
        <div className="container">
          <h1 style={{ fontFamily: 'var(--font-display)', fontSize: '1.5rem', marginBottom: '0.25rem' }}>FAQs</h1>
          <p style={{ color: 'var(--text-muted)', fontSize: '0.9rem' }}>Everything you need to know about LPDR</p>
        </div>
      </div>
      ...
      <section className="section">
        <div className="container" style={{ maxWidth: '800px' }}>
          <div style={{ display: 'flex', flexDirection: 'column', gap: '0.5rem' }}>
            {faqs.map((faq, i) => (
              <div key={i} className="card fade-in">
                ...
                >
                  <div style={{ fontWeight: 600, fontSize: '0.9rem', paddingRight: '1.5rem', color: openId === i ? 'var(--primary)' : 'var(--text-primary)' }}>{faq.q}</div>
                  {openId === i ? <FiChevronUp size={18} style={{ color: 'var(--primary)', flexShrink: 0 }} /> : <FiChevronDown size={18} style={{ color: 'var(--text-muted)', flexShrink: 0 }} />}
                </div>
                {openId === i && (
                  <div style={{ padding: '0 1.25rem 1rem', color: 'var(--text-secondary)', lineHeight: 1.6, fontSize: '0.9rem', borderTop: '1px solid var(--border-subtle)', paddingTop: '0.75rem' }}>
                    {faq.a}
                  </div>
                )}
              </div>
          ...
          <div className="card" style={{ marginTop: '1.5rem', borderLeft: '3px solid var(--primary)' }}>
            <div style={{ padding: '1.25rem', textAlign: 'center' }}>
              <h3 style={{ marginBottom: '0.4rem', fontSize: '1rem' }}>Still have questions?</h3>
              <p style={{ color: 'var(--text-muted)', marginBottom: '1rem', fontSize: '0.85rem' }}>We're here to help. Reach out to our support team.</p>
              <a href="mailto:support@lostpetdronerecovery.com" className="btn btn-primary btn-sm">
                Contact Support
              </a>
            </div>
          </div>
